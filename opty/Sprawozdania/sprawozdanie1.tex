...
\item Porównanie elementów w parze(6 linijka), i posortowanie jej zawsze zajmie $\frac{n}{2}$ porównań.
...
\item Pierwsza pętla \textit{for}(4 linijka) zostanie wykonana dokładnie $\frac{n}{2}$ razy.
\item Porównanie elementów w parze(6 linijka), i posortowanie jej zawsze zajmie $\frac{n}{2}$ porównań.
\item Pierwsza zagnieżdżona pętla \textit{while}(13 linijka) zostanie wykonana $2n$ razy.
\item Druga zagnieżdżona pętla \textit{while}(20 linijka) zostanie wykonana $0$ razy.
\item Jeżeli tablica jest o rozmiarze nieparzystym dodatkowo zostanie wykonane $n$ iteracji(31 linijka).
\end{itemize}
\begin{equation*}
T \approx \frac{n}{2} \cdot(\frac{n}{2} +2n + 0)+ n \approx \frac{5n^2}{4}
\end{equation*}
\wyjT
...
\begin{Huge}
SPRAWDZ ZLOZONOSC PETLI WHILE!
\end{Huge}
\begin{itemize}
\item Pierwsza pętla \textit{for}(4 linijka) zostanie wykonana dokładnie $\frac{n}{2}$ razy.
\item Porównanie elementów w parze(6 linijka), i posortowanie jej zawsze zajmie $\frac{n}{2}$ porównań.
\item Pierwsza zagnieżdżona pętla \textit{while}(13 linijka) zostanie wykonana $\approx \frac{n\cdot(n-1)}{2}$ razy.
\item Druga zagnieżdżona pętla \textit{while}(20 linijka) zostanie wykonana $\approx \frac{n\cdot(n-1)}{2}$ razy.
\item Jeżeli tablica jest o rozmiarze nieparzystym dodatkowo zostanie wykonane $n$ iteracji(31 linijka).
\end{itemize}
\begin{equation*}
\begin{multlined}
T \approx \frac{n}{2} \cdot(n)+ (n-2)
\end{multlined}
\end{equation*}
